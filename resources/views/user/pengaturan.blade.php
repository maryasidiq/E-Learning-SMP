                    ...
                    <div class="card-body box-profile">
                        <div class="text-center">
                            @if (Auth::user()->role == 'Guru')
                                <a href="{{ Auth::user()->guru(Auth::user()->id_card) ? asset(Auth::user()->guru(Auth::user()->id_card)->foto) : asset('img/male.jpg') }}"
                                    data-toggle="lightbox" data-title="Foto {{ Auth::user()->name }}" data-gallery="gallery"
                                    ...
                                    <img src="{{ Auth::user()->guru(Auth::user()->id_card) ? asset(Auth::user()->guru(Auth::user()->id_card)->foto) : asset('img/male.jpg') }}"
                                        width="130px" class="profile-user-img img-fluid img-circle" alt="User profile picture">
                                </a>
                            @elseif (Auth::user()->role == 'Siswa')
                                <a href="{{ Auth::user()->siswa(Auth::user()->no_induk) ? asset(Auth::user()->siswa(Auth::user()->no_induk)->foto) : asset('img/male.jpg') }}"
                                    data-toggle="lightbox" data-title="Foto {{ Auth::user()->name }}" data-gallery="gallery"
                                    data-footer='<a href="{{ route('pengaturan.edit-foto') }}" id="linkFotoGuru" class="btn btn-link btn-block btn-light"><i class="nav-icon fas fa-file-upload"></i> &nbsp; Ubah Foto</a>'>
                        ...
                        @if (Auth::user()->role == 'Guru')
                            <ul class="list-group list-group-unbordered mb-3">
                                ...
                                <li class="list-group-item">
                                    <b>NIP</b> <a
                                        class="float-right">{{ Auth::user()->guru(Auth::user()->id_card) ? Auth::user()->guru(Auth::user()->id_card)->nip : '-' }}</a>
                                </li>
                            </ul>
                        @elseif (Auth::user()->role == 'Siswa')
                            <ul class="list-group list-group-unbordered mb-3">
                                ...
                                <li class="list-group-item">
                                    <b>NIS</b> <a
                                        class="float-right">{{ Auth::user()->siswa(Auth::user()->no_induk) ? Auth::user()->siswa(Auth::user()->no_induk)->nis : '-' }}</a>
                                </li>
                            </ul>
                        @else
                        @endif
                ...
                <div class="card card-primary">
                    ...
                    <div class="card-body">
                        ...
                        @elseif (Auth::user()->role == 'Siswa')
                            ...
                            <p class="text-muted">{{ Auth::user()->siswa(Auth::user()->no_induk) ? Auth::user()->siswa(Auth::user()->no_induk)->kelas->nama_kelas : '-' }}</p>
                            <hr>
                        @else
                        @endif
                        ...
                        @if (Auth::user()->role == 'Guru')
                            ...
                            <p class="text-muted">{{ Auth::user()->guru(Auth::user()->id_card) ? Auth::user()->guru(Auth::user()->id_card)->tmp_lahir : '-' }}</p>
                            <hr>
                        @elseif (Auth::user()->role == 'Siswa')
                            <strong><i class="fas fa-map-marker-alt mr-1"></i> Tempat Lahir</strong>
                            <p class="text-muted">{{ Auth::user()->siswa(Auth::user()->no_induk) ? Auth::user()->siswa(Auth::user()->no_induk)->tmp_lahir : '-' }}</p>
                            <hr>
                        @else
                        @endif
                        ...
                        @if (Auth::user()->role == 'Guru')
                            ...
                            <p class="text-muted">{{ Auth::user()->guru(Auth::user()->id_card) ? Auth::user()->guru(Auth::user()->id_card)->telp : '-' }}</p>
                        @elseif (Auth::user()->role == 'Siswa')
                            <strong><i class="fas fa-phone mr-1"></i> No Telepon</strong>
                            <p class="text-muted">{{ Auth::user()->siswa(Auth::user()->no_induk) ? Auth::user()->siswa(Auth::user()->no_induk)->telp : '-' }}</p>
                        @else
                        @endif
                    </div>
                    <!-- /.card-body -->
