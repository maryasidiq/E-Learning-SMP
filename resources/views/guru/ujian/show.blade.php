...
@section('pageTitle', 'Detail Ujian')
...
@section('content')
    <div class="max-w-6xl mx-auto bg-white dark:bg-white/[0.03] rounded-xl border border-gray-200 dark:border-gray-800 p-6">
        <div class="flex justify-between items-center mb-6">
            <div>
                <h1 class="text-2xl font-bold dark:text-gray-300">{{ $ujian->judul }}</h1>
                @if($ujian->deskripsi)
                    <p class="text-gray-600 dark:text-gray-400 mt-1">{!! $ujian->deskripsi !!}</p>
                @endif
            </div>
            <div class="flex space-x-2">
                <a href="{{ route('ujian.edit', Crypt::encrypt($ujian->id)) }}"
                    class="inline-flex items-center px-4 py-2 bg-yellow-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-yellow-700 focus:bg-yellow-700 active:bg-yellow-900 focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:ring-offset-2 transition ease-in-out duration-150">
                    Edit Ujian
                </a>
                <a href="{{ route('ujian.create-soal', Crypt::encrypt($ujian->id)) }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div class="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
                ...
                <p class="text-lg font-semibold text-gray-900 dark:text-gray-300">{{ $ujian->mapel->nama_mapel }}</p>
            </div>
            <div class="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
                <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400">Kelas</h3>
                <p class="text-lg font-semibold text-gray-900 dark:text-gray-300">{{ $ujian->kelas->nama_kelas }}</p>
            </div>
            <div class="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
            ...
            <div class="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
                ...
                <p class="text-lg font-semibold text-gray-900 dark:text-gray-300">{{ $ujian->durasi }} menit</p>
            </div>
        </div>
        ...
        <div class="mb-6">
                    ...
                    @foreach($soal as $index => $item)
                        <div class="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
                            <div class="flex justify-between items-start mb-2">
                                ...
                                <div class="flex space-x-2">
                                    <a href="{{ route('ujian.edit-soal', [Crypt::encrypt($ujian->id), Crypt::encrypt($item->id)]) }}"
                                    ...
                                    <form
                                        action="{{ route('ujian.destroy-soal', [Crypt::encrypt($ujian->id), Crypt::encrypt($item->id)]) }}"
            ...
            @else
                <div class="text-center py-12 bg-gray-50 dark:bg-gray-800 rounded-lg">
                    ...
                    <p class="mt-1 text-sm text-gray-500">Tambahkan soal pertama untuk ujian ini.</p>
                    <div class="mt-6">
                        <a href="{{ route('ujian.create-soal', Crypt::encrypt($ujian->id)) }}"
