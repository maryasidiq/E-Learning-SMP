...
@section('pageTitle', 'Tambah Soal Ujian')
...
@section('content')
    <div class="max-w-4xl mx-auto bg-white dark:bg-white/[0.03] rounded-xl border border-gray-200 dark:border-gray-800 p-6">
        <h1 class="text-2xl mb-6 dark:text-gray-300">Tambah Soal untuk Ujian: {{ $ujian->judul }}</h1>
        ...
        <!-- Form soal dinamis -->
        <form id="soal_form" action="{{ route('ujian.store-soal', Crypt::encrypt($ujian->id)) }}" method="POST"
            style="display: none;">
            @csrf
    ...
    <script>
        ...
        function generateSoalFromPDF() {
            ...
            fetch('/guru/ujian/generate-soal-pdf', {
                method: 'POST',
                body: formData
            })
        ...
        function pollForResults(cacheKey) {
            ...
            const pollInterval = setInterval(() => {
                fetch('/guru/ujian/check-generate-status', {
                    method: 'POST',
                    headers: {
