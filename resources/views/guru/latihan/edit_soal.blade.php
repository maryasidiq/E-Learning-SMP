...
@section('content')
    <div class="max-w-4xl mx-auto bg-white dark:bg-white/[0.03] rounded-xl border border-gray-200 dark:border-gray-800 p-6">
        <h1 class="text-2xl mb-6 dark:text-gray-300">Edit Soal untuk Quis: {{ $quis->judul }}</h1>

        <form action="{{ route('quis.updateSoal', [Crypt::encrypt($quis->id), Crypt::encrypt($soal->id)]) }}" method="POST">
            @csrf
            @method('PUT')
