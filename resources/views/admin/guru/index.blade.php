        ...
        <div class="card">
            ...
            <div class="card-body">
                <table id="example1" class="table table-bordered table-striped table-hover">
                    ...
                    <tbody>
                        @foreach ($guruMapels as $data)
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td>{{ $data['guru']->nama_guru }}</td>
                                <td>
                                    @foreach ($data['mapels'] as $mapelItem)
                                        {{ $mapelItem['paket_id'] }}<br>
                                    @endforeach
                                </td>
                                <td>
                                    @foreach ($data['mapels'] as $mapelItem)
                                        {{ $mapelItem['nama_mapel'] }} ({{ $mapelItem['kelompok'] }})<br>
                                    @endforeach
                                </td>
                                <td>
                                    <a href="{{ route('guru.show', Crypt::encrypt($data['guru']->id)) }}"
                                        class="btn btn-info btn-sm"><i class="nav-icon fas fa-id-card"></i> &nbsp; Detail</a>
                                    <a href="{{ route('guru.edit', Crypt::encrypt($data['guru']->id)) }}"
