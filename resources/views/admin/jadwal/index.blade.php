              ...
              <tr>
                ...
                <td>
                  <a href="{{ route('jadwal.show', Crypt::encrypt($data->id)) }}" class="btn btn-info btn-sm"><i
                      class="nav-icon fas fa-search-plus"></i> &nbsp; Details</a>
                </td>
              </tr>
            @endforeach
          </tbody>
...
@section('script')
  <script>
    ...
    // AJAX to load mapel based on kelas selection
    $('#kelas_id').change(function () {
      var kelas_id = $(this).val();
      if (kelas_id) {
        $.ajax({
          url: '/admin/jadwal/get-mapel-by-kelas/' + kelas_id,
          type: 'GET',
          success: function (data) {
            $('#mapel_id').empty();
            $('#mapel_id').append('<option value="">-- Pilih Mapel --</option>');
            $.each(data, function (key, value) {
              $('#mapel_id').append('<option value="' + value.id + '">' + value.nama_mapel + '</option>');
            });
          }
        });
      } else {
      ...
      // Reset guru when kelas changes
      $('#guru_id').empty();
      $('#guru_id').append('<option value="">-- Pilih Guru --</option>');
    });

    // AJAX to load guru based on mapel selection
    $('#mapel_id').change(function () {
      var mapel_id = $(this).val();
      if (mapel_id) {
        $.ajax({
          url: '/admin/jadwal/get-guru-by-mapel/' + mapel_id,
          type: 'GET',
          success: function (data) {
            $('#guru_id').empty();
            $('#guru_id').append('<option value="">-- Pilih Guru --</option>');
            $.each(data, function (key, value) {
              $('#guru_id').append('<option value="' + value.id + '">' + value.nama_guru + '</option>');
            });
          }
        });
      } else {
